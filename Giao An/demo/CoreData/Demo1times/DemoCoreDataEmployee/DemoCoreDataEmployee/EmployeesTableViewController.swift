...
class EmployeesTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? EmployeeTableViewCell else { fatalError("Error") }
        let emp = arrayEmp[indexPath.row]
        guard let url = URL(string: String(describing: emp.imageEmp)) else { fatalError("Khong lay duoc url") }
        guard let data = try? Data(contentsOf: url) else { fatalError("Khong xu ly dk noi dung url") }
        let image: UIImage = UIImage(data: data)!
        cell.idEmp.text = emp.idEmp
        cell.nameEmp.text = emp.nameEmp
        cell.photoEmp.image = image
